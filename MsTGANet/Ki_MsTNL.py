...
class SPP_Q(nn.Module):
    def __init__(self, in_ch, out_ch, up_scale, ks=3):
        ...
        # Changed name to "Up" from down, and variable name from down_scale to "up_scale"
        self.Up = nn.Upsample(scale_factor=up_scale, mode="bilinear")
...
class Encoder_Pos(nn.Module):
    def __init__(self, n_dims, width=32, height=32, filters=[32, 64, 128, 256]):
        ...
        # Changes made to input layers to up-sample instead of down-sample in order to match layer 5
        # Up-scale: 16x
        self.SPP_Q_0 = SPP_Q(in_ch=filters[0], out_ch=n_dims, up_scale=16, ks=3)
        # Up-scale: 8x
        self.SPP_Q_1 = SPP_Q(in_ch=filters[1], out_ch=n_dims, up_scale=8, ks=3)
        # Up-scale: 4x
        self.SPP_Q_2 = SPP_Q(in_ch=filters[2], out_ch=n_dims, up_scale=4, ks=3)
        # Up-scale: 2x
        self.SPP_Q_3 = SPP_Q(in_ch=filters[3], out_ch=n_dims, up_scale=2, ks=3)

...
class Downsampling(nn.Module):
    def __init__(self, in_ch, out_ch, down_scale, kernel_size=3):
        super().__init__()
        # noinspection PyArgumentEqualDefault
        self.Conv = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=kernel_size, stride=1, padding=kernel_size // 2),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
        self.downsample = nn.Upsample(scale_factor=down_scale, mode="bilinear")
...
class Encoder_Pos(nn.Module):
    ...
    def __init__(self, n_dims, width=32, height=32, filters=[32, 64, 128, 256]):
        ...
        self.SPP_Q_0 = Downsampling(in_ch=filters[0], out_ch=n_dims, down_scale=1 / 16, kernel_size=3)
        self.SPP_Q_1 = Downsampling(in_ch=filters[1], out_ch=n_dims, down_scale=1 / 8, kernel_size=3)
        self.SPP_Q_2 = Downsampling(in_ch=filters[2], out_ch=n_dims, down_scale=1 / 4, kernel_size=3)
        self.SPP_Q_3 = Downsampling(in_ch=filters[3], out_ch=n_dims, down_scale=1 / 2, kernel_size=3)
...
class Decoder_Pos(nn.Module):
    def __init__(self, channels_in, width=32, height=32):
        super(Decoder_Pos, self).__init__()
        print("================= Multi_Head_Decoder =================")

        self.channels_in = channels_in
        self.rel_h = nn.Parameter(torch.randn([1, channels_in // 8, height, 1]), requires_grad=True)
        self.rel_w = nn.Parameter(torch.randn([1, channels_in // 8, 1, width]), requires_grad=True)
        self.query_conv = nn.Conv2d(in_channels=channels_in, out_channels=channels_in // 8, kernel_size=1)
        self.key_conv = nn.Conv2d(in_channels=channels_in, out_channels=channels_in // 8, kernel_size=1)
        self.value_conv = nn.Conv2d(in_channels=channels_in, out_channels=channels_in, kernel_size=1)
    ...
    def forward(self, x, x_encoder):
        ...
        content_position = (self.rel_h + self.rel_w).view(1, self.channels_in // 8, -1)
        content_position = torch.matmul(proj_query, content_position)
